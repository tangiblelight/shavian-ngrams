...
script = flair = spaCy = alphabet = notags = False
dict = htags = { }
tokens = [ "." ]
units = { "ms":"𐑥𐑕","bc":"𐑚𐑰𐑕𐑰","psi":"𐑐𐑕𐑦","pc":"𐑐𐑕","mi":"𐑥𐑲" }
contr = [ "'d","'ll","'m","n't","'re","'s","'ve" ]
abbrev = [ "abbr","acad","al","alt","apr","assn","at","aug","ave","b","c","cf",
	"capt","cent","chm","chmn","co","col","comdr","corp","cpl","d","dec","dept","dist",
...
def tokenize (str):
  ...
  for i in range(1,len(str)-1):
    new = 0
    if str[i].isalpha(): new = 1
    if str[i].isdigit(): new = 2
    if str[i] == " " and  tokens[-1][0].isalpha() and str[i+1].isalpha(): new = 0
    if str[i] in "'." and str[i-1].isalpha() and str[i+1].isalpha(): new = 1
    if str[i] in ",." and str[i-1].isdigit() and str[i+1].isdigit(): new = 2
    if str[i] == "." and old == 1 and "." in tokens[-1]: new = 1	     # U.S.A. keeps the dot
    if str[i] == "." and new == 0 and tokens[-1].lower() in abbrev: continue # Dr. Mr. etc. lose it
    if old and old == new: tokens[-1] += str[i]
    else:
...
def suffix_split (inp, pos, adj):
  ...
  for split in range(2,long):
    ...
    if low[split:] == "ha" and low[split-1] in "cst": continue
    ...
    if low[split:] == "n" and low[split-1] in "eio" \
		and low[split-2] != low[split-1]: continue
    if low[split:] == "ess" and low[split-1] in "ln" \
		and low[split-2] == low[split-1]: continue
    suff = dict[suff]
    for pess in range(2):
      if (pess) or inp[split] == "'":
        ...
        if low[split-1] == low[split-2] and low[split-1] not in "hsw":
          word = inp[:split-1]
        elif ( low[split-1] in "cghlsuvz" or low[split] == 'e' or low[split-2] in "aeiousy" ) \
	  ...
	  and low[split:] not in [ "arm","out","und","up" ]:
        ...
        root += "𐑦"
      if root[-1] == "𐑓" and suff[0] > "𐑗" and word[-1] in "vw" and low[split:] != "s":
        root = root[:-1] + "𐑝"
  ...
  if len(best[1]) > 1:
    ...
    if word[-4:] == "𐑒𐑩𐑤𐑦" and word[-5] in "𐑦𐑩":
      word = word[:-4] + "𐑒𐑤𐑦"
    if word[-1] == "𐑾": word += "0"
    if word[-2] == "𐑾" and len(word) > 4 and word[-3] in "𐑑𐑕𐑟" \
	and word[-1] in "𐑕𐑤𐑯0":
      mid = "𐑖"
      if word[-3] == "𐑑":
        if word[-4] == "𐑕": mid = "𐑗"
      elif word[-1] in "𐑯0" and word[-4] in "𐑰𐑱𐑴𐑵𐑷𐑻𐑿": mid = "𐑠"
      word = word[:-3] + mid + "𐑩" + word[-1]
    if word[-1] == "0": word = word[:-1]
    best = (best[0], word)
  return best
...
def prefix_split (word, pos, ms):
  ...
  for split in range(len(word)-2,ms,-1):
    ...
    if score > best[0]:
      ...
      pref = dict[pref]
      if pref[-1] == root[1][0] and pref[-1] in "𐑤𐑥𐑮𐑯" and pref[-2] == '𐑦' \
	or pref == "𐑥𐑩𐑒" and (root[1][0] == "𐑒" or root[1][:2] == "·𐑒"):
        pref = pref[:-1]
...
for fname in sys.argv[1:]:
  ...
  if fname == "-s":	# Use spaCy for POS tagging
    ...
    continue
  if fname == "-d":	# output Deseret
    alphabet = 1
    continue
  if fname == "-i":	# output IPA
    alphabet = 2
    continue
  if fname == "-r":	# output Runic
    alphabet = 3
    apostrophe = "ᛌ"
    continue
  if fname == "-n":
...
if alphabet == 1: map.update({"𐑩":"𐑱","𐑩𐑯":"𐑨𐑯","𐑝":"𐑪𐑝"})
else: map["𐑞"] = "𐑞𐑩"
...
#  print (token, file=sys.stderr)
  ...
  if prev[0] in befto and low == "to":
    if notags: out.extend(["@",befto[prev[0]]," "])
  ...
  if word[0].isalpha():
    ...
    if root[1]: tran = root[1].replace("'",apostrophe).replace(":","")
    if prev[0][0].isdigit() and low in units:
      tran = units[low]
    if not alphabet: dot = maydot and "·" in tran
    tran = tran.replace("·","")
    if alphabet and tran in map: tran = map[tran]
    if alphabet == 1:
    ...
    if alphabet == 3:
      for tup in [("𐑦𐑹","𐑦𐑷𐑮"),("𐑣𐑿","𐑣𐑘𐑵"),("𐑒𐑢","ᛢ"),("𐑕𐑗","ᛥᛄ"),("𐑕𐑑","ᛥ"),("𐑣𐑤","ᚻ\u200dᛚ")]:
        tran = tran.replace(tup[0],tup[1])
      tran = re.sub("([𐑦𐑰][𐑪𐑴𐑷]|𐑣[𐑒𐑘𐑜])","ᛇ",tran)
    if alphabet and low[-2:] not in [ "id","iz","zz" ]:
      for i in range(3,len(tran)):
        if tran[i] == "𐑦" and (tran[i+1:] in [ "","𐑛","𐑟" ] or tran[i+1] == "𐑦"):
...
if alphabet:
  map = [ [ "𐐹","𐐻","𐐿","𐑁","𐑃","𐑅","𐑇","𐐽","𐐷","𐑍",
	  ...
	  [ "ᛈ","ᛏ","ᛣ","ᚠ","ᚦ","ᛋ","ᛋᚳ","ᚳ","ᛄ","ᛝ",
	    "ᛒ","ᛞ","ᚸ","ᚠ\u200dᚠ","ᚦ","ᛉ","ᛉᚳ","ᚷ","ᚹ","ᚻ",
	    "ᛚ","ᛗ","ᛁ","ᛖ" ,"ᚫ","ᛟ","ᚩ","ᚢ","ᚣ","ᚪ",
	    "ᚱ","ᚾ","ᛁ","ᛖᛡ","ᚪᛡ","ᚪ","ᚩᚢ","ᚢ","ᚩᛡ","ᚩ",
	    "ᚪᚱ","ᚩ\u200dᚱ","ᛖ\u200dᚱ","ᛟᚱ","ᛟᚱ","ᛠᚱ","ᛠ","ᛄᚢ","ᛁ" ],
	][alphabet-1]
  cmap = { }
  if alphabet == 3:
    cmap = {" ":"᛫","-":"᛫",",":"᛫᛫",".":"᛬","!":"᛭",
  ...
  for i in range(len(inp)-2):
    ...
    if char >= "𐑐" and char <= "𐒀":
      ...
      if inp[i] == "·": char = char[0].upper() + char[1:]
    if inp[i] == "<": content = False
    if inp[i] == ">": content = True
    if inp[i] in "𐑦𐑪𐑫𐑳":
      if char in "ᛈᛋᛒᛞᚸᛉᚷᚹᛗᚱᚾ": char = char + "\u200d" + char
      elif char in "ᛏᚠᚦᚳᛚ":     char = char + char
      elif char == "ᛣ":         char = "ᛤ"
    if inp[i+2] in "𐑤𐑮𐑘𐑿ᛄ" and char == "ᛣ": char = "ᚳ"
    if content and char in cmap:
      ...
      if char == '"':
        ...
        dquote = not dquote
      char = cmap[char] + "\u200b"
      if inp[i] in cmap and inp[i+1] == " ": char = ""
    if char != "·": out.append(char)
  out = "".join(out)
  for tup in [("ᛁᛁ","ᛡᛁ"),("ᛁᛥᛄ","ᛁᛋ\u200dᛋᚳ"),("ᚾ\u200dᚾᛏ","ᚾᚾᛏ"),("ᚾᛏ","ᚾ\u200dᛏ"),("ᚣᛟᚱ","ᚣᚱ"),("ᚢᛟᚱ","ᚢᚱ"),("ᚪᛡᛟᚱ","ᚪᛡᚱ"),("ᛋᚳᛟᚾ","ᛋᚳᚾ"),("ᛋᚳᛄ","ᛋᛣᛄ"),("ᛉᚳᛟᚾ","ᛉᚳᚾ"),("ᛤᛥ","ᛤᛋᛏ")]:
